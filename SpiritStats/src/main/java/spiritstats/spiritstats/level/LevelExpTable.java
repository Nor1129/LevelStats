...
public class LevelExpTable {
    ...
    private static final int[] LEVEL_XP = new int[] {
            // 0 ~ 11
            30, 50, 75, 100, 130, 170, 220, 280, 350, 430, 500,
    ...
    public static int getRequiredExp(int level) {
        if (level < 0 || level >= LEVEL_XP.length) {
            throw new IllegalArgumentException("Level must be between 0 and " + (LEVEL_XP.length - 1));
        }
